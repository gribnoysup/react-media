...
const createMockMediaMatcher = matchesOrMapOfMatches => qs => ({
  matches:
    matchesOrMapOfMatches && typeof matchesOrMapOfMatches === "object"
...
describe("A <Media> in browser environment", () => {
    ...
    describe("and a render prop", () => {
      it("should render `render` prop call result", () => {
        const element = (
          <Media
            queries={queries}
            render={matches => matches.sm && matches.lg && <span>render prop</span>}
          />
        )
  ...
  describe("with a query that partially match", () => {
    ...
    describe("and a child component", () => {
      it("should render child and provide matches as a prop", () => {
        const Component = props =>
          props.matches.sm && !props.matches.lg && <span>partially matched</span>
    ...
    describe("and a render prop", () => {
      it("should render `render` prop call result", () => {
        const element = (
          <Media
            queries={queries}
            render={matches => matches.sm && !matches.lg && <span>please render me</span>}
          />
        )
